...
)
{
  ...
  {
    val iOptional = pattern.indexWhere(Seq(ZERO_OR_ONE, ZERO_OR_MORE).contains)
    if (iOptional == -1) {
    ...
    cycleLinker : Option[CycleLinker])
  {
    if (pattern.isEmpty) {
      ...
      if (symbol == CYCLE_END) {
        ...
        children.put(ONE_OR_MORE, cycleTrie)
        ...
        val (remainder, newLinker, insert) = patternTail.headOption match {
          case Some(ONE_OR_MORE) => {
            ...
            tupleN((patternTail.tail,
              Some(CycleLinker(cycleTrie, Some(cycleTrie))), Seq(CYCLE_END)))
          }
          case _ => {
